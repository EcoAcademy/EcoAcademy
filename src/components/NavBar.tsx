...
const NavBar: React.FC = () => {
  ...
  const performSearch = (query: string): SearchResult[] => {
    const data: SearchResult[] = [
      ...
      { id: 2, name: 'Project-Based Learning', description: 'Hands-on projects designed to enhance practical skills.' },
      { id: 3, name: 'Teacher Training Programs', description: 'Programs for educators to improve teaching methodologies.' },
      { id: 4, name: 'Parent Engagement Workshops', description: 'Workshops for parents to support their children’s education.' },
      { id: 5, name: 'Student Success Stories', description: 'Highlighting student achievements through EcoAcademy’s programs.' },
    ];
      ...
      <nav className="bg-green-600 p-4 shadow-lg">
        <div className="container mx-auto flex justify-between items-center">
          ...
          <div className="hidden md:flex space-x-8">
            <Link href="/students/play" className="text-green-400 hover:text-green-500">Play</Link>
            <Link href="/students" className="text-green-400 hover:text-green-500">Students</Link>
            <Link href="/teachers" className="text-green-400 hover:text-green-500">Teachers</Link>
            <Link href="/parents" className="text-green-400 hover:text-green-500">Parents</Link>
            <Link href="/about" className="text-green-400 hover:text-green-500">About Us</Link>
          </div>
        ...
        {isMenuOpen && (
          <div className="md:hidden mt-4 flex flex-col items-center space-y-4">
            <Link href="/students/play" className="text-white hover:text-gray-200">Play</Link>
            <Link href="/students" className="text-white hover:text-gray-200">Students</Link>
            <Link href="/teachers" className="text-white hover:text-gray-200">Teachers</Link>
            <Link href="/parents" className="text-white hover:text-gray-200">Parents</Link>
            ...
            <form onSubmit={(e) => e.preventDefault()} className="flex items-center mt-4">
              ...
              <button
                type="button"
                className="bg-green-500 text-white px-4 py-2 rounded-full ml-2 hover:bg-green-600 transition-colors duration-300"
              >
                Search
              </button>
            </form>
